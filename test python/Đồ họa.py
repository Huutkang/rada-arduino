...
def xyz(n):
    ...
    # pygame.draw.rect(be_mat4, "blue", [0, 0, 700, 50])
    be_mat5 = so(n)
...
def so(n):
    ...
    # pygame.draw.rect(be_mat, "red", [0, 0, 50, 50])
...
def tt ():
    global goc_hien_tai, huong_quay
    goc_hien_tai = goc_hien_tai + huong_quay
    if goc_hien_tai > 165:
        goc_hien_tai = 165
        huong_quay = -1
    if goc_hien_tai < 15:
...
pygame.display.set_caption('Hệ Thống Nhúng')
